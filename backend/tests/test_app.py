...
class TestChunkingService(unittest.TestCase):
    def test_fixed_size_chunking(self):
        text = "1234567890"
        chunks = RuleBasedChunker.chunk_by_fixed_size(text, chunk_size=5, chunk_overlap=0)
    ...
    def test_fixed_size_chunking_overlap(self):
        text = "1234567890"
        chunks = RuleBasedChunker.chunk_by_fixed_size(text, chunk_size=5, chunk_overlap=2)
...
class TestAPI(unittest.TestCase):
    ...
    def test_process_endpoint(self, mock_process):
        ...
        response = self.client.post("/api/v1/process/", json={
            "text": "some text",
            "chunking_options": {
                "method": "fixed_size",
                "chunk_size": 100,
